...
def reply_test(reply_df,file,df_hist):
    ...
    for i in range(1,reply_df_len):
        df = reply_df.head(i).tail(1)
    ...
    try:
        if stock_fenshi == '120':  # 只在120分时下刷新df
            ...
            if re_try_count == run_time_args_dic['最大重试次数']:
                print_and_log('重试超过' + str(re_try_count) + '次-----'+"股票代码：" + stock_code + " 股票名称:  " + stock_name)
                return 0
            df.to_csv(run_time_args_dic['临时数据'] + "/df_1min_hist_list_" + stock_code + '_' + stock_name + '_.csv',index=False, encoding="utf-8-sig")
        else:  # 其他分时直接读取
            df = pd.read_csv(run_time_args_dic['临时数据'] + "/df_1min_hist_list_" + stock_code + '_' + stock_name + '_.csv')
    except Exception as e:
...
if __name__ == '__main__':
    ...
    if run_time_args_dic['reply_mode'] == 1:
        hist_file = 'df_5min_hist_list_002677_测试_.csv' #测试用的base历史数据
        reply_file = 'df_1min_hist_list_002677_测试_.csv'
        stock_code = hist_file.split('_')[4]
        stock_fenshi = hist_file.split('_')[1].split('min')[0]

        df = ak.stock_zh_a_hist_min_em(symbol=stock_code,period=stock_fenshi,adjust='qfq',start_date=  "1979-09-01 09:32:00",end_date = "2025-02-27 15:00:00")
        df['底背离'] = 'Nothing'
        df['顶背离'] = 'Nothing'
    ...
    while 1:
        while 1:
            if datetime.now().second == 0 and trad_time_check():  #等待整分
                time.sleep(2)
